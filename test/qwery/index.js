...
module.exports = {
    Contexts: {
        "should be able to pass optional context"() {
            ...
            expect(
                CSSselect.selectAll(".a", CSSselect.selectAll("#boosh", document))
...
*/
        "should not return duplicates from combinators"() {
            expect(CSSselect.selectAll("#boosh,#boosh", document)).to.have.length(1); //two booshes dont make a thing go right
            expect(CSSselect.selectAll("#boosh,.apples,#boosh", document)).to.have.length(
                1
        ...
        "byId sub-queries within context"() {
            expect(
                CSSselect.selectAll("#booshTest", CSSselect.selectAll("#boosh", document))
            ).to.have.length(1); //found "#id #id"
            expect(
                CSSselect.selectAll(".a.b #booshTest", CSSselect.selectAll("#boosh", document))
            ).to.have.length(1); //found ".class.class #id"
            expect(
                CSSselect.selectAll(".a>#booshTest", CSSselect.selectAll("#boosh", document))
            ).to.have.length(1); //found ".class>#id"
            expect(
                CSSselect.selectAll(">.a>#booshTest", CSSselect.selectAll("#boosh", document))
            ).to.have.length(1); //found ">.class>#id"
            expect(
                CSSselect.selectAll("#boosh", CSSselect.selectAll("#booshTest", document)).length
            ).to.not.be.ok(); //shouldn't find #boosh (ancestor) within #booshTest (descendent)
            expect(
                CSSselect.selectAll("#boosh", CSSselect.selectAll("#lonelyBoosh", document)).length
    ...
    "CSS 1": {
        ...
        "byId sub-queries"() {
            expect(CSSselect.selectAll("#boosh #booshTest", document)).to.have.length(1); //found "#id #id"
            expect(CSSselect.selectAll(".a.b #booshTest", document)).to.have.length(1); //found ".class.class #id"
            expect(CSSselect.selectAll("#boosh>.a>#booshTest", document)).to.have.length(
                1
            ); //found "#id>.class>#id"
            expect(CSSselect.selectAll(".a>#booshTest", document)).to.have.length(1); //found ".class>#id"
        },

        "get elements by class"() {
            expect(CSSselect.selectAll("#boosh .a", document)).to.have.length(2); //found two elements
            expect(CSSselect.selectAll("#boosh div.a", document)[0]).to.be.ok(); //found one element
            expect(CSSselect.selectAll("#boosh div", document)).to.have.length(2); //found two {div} elements
            expect(CSSselect.selectAll("#boosh span", document)[0]).to.be.ok(); //found one {span} element
            expect(CSSselect.selectAll("#boosh div div", document)[0]).to.be.ok(); //found a single div
        ...
        "class with dashes"() {
            expect(CSSselect.selectAll(".class-with-dashes", document)).to.have.length(1); //found something
        },
        ...
        "deep messy relationships"() {
            ...
            expect(CSSselect.selectAll("div#fixtures > div a", document)).to.have.length(
                5
            ); //found four results for "div#fixtures > div a"
            expect(
                CSSselect.selectAll(".direct-descend > .direct-descend .lvl2", document)
            ).to.have.length(1); //found one result for ".direct-descend > .direct-descend .lvl2"
            expect(
                CSSselect.selectAll(".direct-descend > .direct-descend div", document)
    ...
    "CSS 2": {
        "get elements by attribute"() {
            ...
            expect(CSSselect.selectAll("#boosh div[test=fg]", document)[0]).to.be(
                expected
            ); //found attribute with value
            expect(CSSselect.selectAll('em[rel~="copyright"]', document)).to.have.length(
                1
        ...
        "should not throw error by attribute selector"() {
            expect(CSSselect.selectAll('[foo^="bar"]', document)).to.have.length(1); //found 1 element
        },
    ...
    "attribute selectors": {
        ...
        "[attr=val]"() {
            ...
            expect(
                CSSselect.selectAll('#attributes div[test="two-foo"]', document)[0]
        ...
        "[attr|=val]"() {
            ...
            expect(CSSselect.selectAll("#attributes div[test|=two]", document)[0]).to.be(
                expected
        ...
        "[attr$=val]"() {
            const expected = DomUtils.getElementById("attr-test-2", document);
            expect(CSSselect.selectAll("#attributes div[test$=foo]", document)[0]).to.be(
                expected
            ); //found attribute with $=
        },
        ...
        "[attr*=val]"() {
            const expected = DomUtils.getElementById("attr-test-3", document);
            expect(CSSselect.selectAll("#attributes div[test*=hree]", document)[0]).to.be(
                expected
            ); //found attribute with *=
        },
        ...
        "direct descendants"() {
            expect(
                CSSselect.selectAll("#direct-descend > .direct-descend", document)
        ...
        "sibling elements"() {
            expect(
                CSSselect.selectAll("#sibling-selector ~ .sibling-selector", document)
            ).to.have.length(2); //found two siblings
            expect(
                CSSselect.selectAll("#sibling-selector ~ div.sibling-selector", document)
            ).to.have.length(2); //found two siblings
            expect(
                CSSselect.selectAll("#sibling-selector + div.sibling-selector", document)
            ...
            expect(CSSselect.selectAll(".parent .middle ~ h4", document)).to.have.length(
                1
            ...
            expect(CSSselect.selectAll(".parent .middle ~ h3", document)).to.be.empty(); //an element can't be its own sibling
            expect(CSSselect.selectAll(".parent .middle ~ h2", document)).to.be.empty(); //didn't find an older sibling
            expect(
...
*/
    ...
    "element-context queries": {
        "relationship-first queries"() {
            ...
            expect(
                CSSselect.selectAll("> .tokens a", CSSselect.selectAll(".idless", document)[0])
        ...
        "byId sub-queries within detached fragment"() {
            ...
            expect(CSSselect.selectAll(".d #oooo #emem", frag)).to.have.length(1); //found ".class #id #id" in fragment
            expect(CSSselect.selectAll("> div #oooo", frag)).to.have.length(1); //found "> .class #id" in fragment
            expect(
                CSSselect.selectAll("#oooo", CSSselect.selectAll("#emem", frag)).length
            ).to.not.be.ok(); //shouldn't find #oooo (ancestor) within #emem (descendent)
            expect(
                CSSselect.selectAll("#sep", CSSselect.selectAll("#emem", frag)).length
        ...
        "forms can be used as contexts"() {
            expect(
                CSSselect.selectAll("*", CSSselect.selectAll("form", document)[0])
    ...
    tokenizer: {
        "should not get weird tokens"() {
            expect(CSSselect.selectAll('div .tokens[title="one"]', document)[0]).to.be(
            ...
            expect(
                CSSselect.selectAll('div .tokens[title="one two three #%"]', document)[0]
    ...
    "interesting syntaxes": {
        "should parse bad selectors"() {
            expect(
                CSSselect.selectAll("#spaced-tokens    p    em    a", document).length
    ...
    "order matters": {
        ...
        "the order of elements return matters"() {
            ...
            const els = CSSselect.selectAll("#order-matters .order-matters", document);
    ...
    "pseudo-selectors": {
        ":contains"() {
            expect(CSSselect.selectAll("li:contains(humans)", document)).to.have.length(
                1
            ); //found by "element:contains(text)"
            expect(CSSselect.selectAll(":contains(humans)", document)).to.have.length(5); //found by ":contains(text)", including all ancestors
            // * is an important case, can cause weird errors
            expect(CSSselect.selectAll("*:contains(humans)", document)).to.have.length(5); //found by "*:contains(text)", including all ancestors
            expect(CSSselect.selectAll("ol:contains(humans)", document)).to.have.length(
                1
            ); //found by "ancestor:contains(text)"
        },

        ":not"() {
            expect(CSSselect.selectAll(".odd:not(div)", document)).to.have.length(1); //found one .odd :not an &lt;a&gt;
        },

        ":first-child"() {
            expect(CSSselect.selectAll("#pseudos div:first-child", document)[0]).to.be(
                pseudos[0]
            ); //found first child
            expect(
        ...
        ":last-child"() {
            ...
            expect(CSSselect.selectAll("#pseudos div:last-child", document)[0]).to.be(
                all[all.length - 1]
            ); //found last child
            expect(
        ...
        'ol > li[attr="boosh"]:last-child'() {
            ...
            expect(
                CSSselect.selectAll('ol > li[attr="boosh"]:last-child', document)
            ).to.have.length(1); //only 1 element found
            expect(
                CSSselect.selectAll('ol > li[attr="boosh"]:last-child', document)[0]
        ...
        ":nth-child(odd|even|x)"() {
            ...
            expect(CSSselect.selectAll("#pseudos div:nth-child(2)", document)[0]).to.be(
                second
        ...
        ":nth-child(expr)"() {
            ...
            expect(CSSselect.selectAll("#pseudos :nth-child(3n+2)", document)[1]).to.be(
                fifth
            ); //second :nth-child(3n+2) is the fifth child
            expect(CSSselect.selectAll("#pseudos :nth-child(3n)", document)[1]).to.be(
                sixth
        ...
        ":nth-last-child(odd|even|x)"() {
            ...
            expect(
                CSSselect.selectAll("#pseudos div:nth-last-child(odd)", document)
            ).to.have.length(3); //found 3 odd elements with div tag
            expect(
                CSSselect.selectAll("#pseudos div:nth-last-child(even)", document)
            ).to.have.length(3); //found 3 even elements with div tag
            expect(
                CSSselect.selectAll("#pseudos div:nth-last-child(6)", document)[0]
        ...
        ":nth-last-child(expr)"() {
            ...
            expect(
                CSSselect.selectAll("#pseudos :nth-last-child(3n+2)", document)[0]
        ...
        ":nth-last-of-type(expr)"() {
            ...
            expect(
                CSSselect.selectAll("#pseudos div:nth-last-of-type(3n+1)", document)
            ).to.have.length(2); //found 2 div elements
            expect(
                CSSselect.selectAll("#pseudos a:nth-last-of-type(3n+1)", document)
            ).to.have.length(1); //found 1 a element
            expect(
                CSSselect.selectAll("#pseudos div:nth-last-of-type(5)", document)[0]
            ).to.be(second); //5th nth-last-of-type should be 2nd of 7 elements
        },

        ":first-of-type"() {
            expect(CSSselect.selectAll("#pseudos a:first-of-type", document)[0]).to.be(
        ...
        ":last-of-type"() {
            const all = DomUtils.getElementsByTagName("div", pseudos);
            expect(CSSselect.selectAll("#pseudos div:last-of-type", document)[0]).to.be(
                all[all.length - 1]
        ...
        ":only-of-type"() {
            expect(CSSselect.selectAll("#pseudos a:only-of-type", document)[0]).to.be(
                DomUtils.getElementsByTagName("a", pseudos)[0]
        ...
        ":target"() {
            location.hash = "";
            expect(CSSselect.selectAll("#pseudos:target", document)).to.be.empty(); //#pseudos is not the target
            location.hash = "#pseudos";
            expect(CSSselect.selectAll("#pseudos:target", document)).to.have.length(1); //now #pseudos is the target
            location.hash = "";
        },

        "custom pseudos"() {
            // :humanoid implemented just for testing purposes
            expect(CSSselect.selectAll(":humanoid", document)).to.have.length(2); //selected using custom pseudo
        }
    },
...
*/
    ...
    "is()": {
        ...
        pseudos() {
            ...
            expect(
                CSSselect.is(CSSselect.selectAll("#list>li", document)[2], ":humanoid")
            ).to.be.ok(); //matching custom pseudo
            expect(
                CSSselect.is(CSSselect.selectAll("#list>li", document)[1], ":humanoid")
    ...
    "selecting elements in other documents": {
        ...
        "complex selectors"() {
            ...
            expect(CSSselect.selectAll("#hsoob > div > .h", doc)).to.have.length(1); //found span using child selectors
            expect(CSSselect.selectAll('.a .d ~ .sib[test="f g"]', doc)).to.have.length(
                1
            ); //found 1 ~ sibling with test attribute
        },

        "byId sub-queries"() {
            expect(CSSselect.selectAll("#hsoob #spanny", doc)).to.have.length(1); //found "#id #id" in frame
            expect(CSSselect.selectAll(".a #spanny", doc)).to.have.length(1); //found ".class #id" in frame
            expect(CSSselect.selectAll(".a #booshTest #spanny", doc)).to.have.length(1); //found ".class #id #id" in frame
            expect(CSSselect.selectAll("> #hsoob", doc)).to.have.length(1); //found "> #id" in frame
        },

        "byId sub-queries within sub-context"() {
            expect(
                CSSselect.selectAll("#spanny", CSSselect.selectAll("#hsoob", doc))
            ).to.have.length(1); //found "#id -> #id" in frame
            expect(
                CSSselect.selectAll(".a #spanny", CSSselect.selectAll("#hsoob", doc))
            ).to.have.length(1); //found ".class #id" in frame
            expect(
                CSSselect.selectAll(".a #booshTest #spanny", CSSselect.selectAll("#hsoob", doc))
            ).to.have.length(1); //found ".class #id #id" in frame
            expect(
                CSSselect.selectAll(".a > #booshTest", CSSselect.selectAll("#hsoob", doc))
            ).to.have.length(1); //found "> .class #id" in frame
            expect(
                CSSselect.selectAll("#booshTest", CSSselect.selectAll("#spanny", doc)).length
            ).to.not.be.ok(); //shouldn't find #booshTest (ancestor) within #spanny (descendent)
            expect(
                CSSselect.selectAll("#booshTest", CSSselect.selectAll("#lonelyHsoob", doc)).length
